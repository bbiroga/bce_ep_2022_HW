...
def F_F_gen_proportions(pivot_F_F_proportions):
    ...
    for age in df_pivot_F_F_proportions.index:
            same_gen[age]= sum(df_pivot_F_F_proportions.loc[age,max(age-5,15):min(age+5,50)])

    for age in df_pivot_F_F_proportions.index:
        if age <=40:
            older_gen[age]= sum(df_pivot_F_F_proportions.loc[age,age+10:min(age+20,50)])
    ...
    for age in df_pivot_F_F_proportions.index:
        if age >=25:
            younger_gen[age]= sum(df_pivot_F_F_proportions.loc[age,max(age-20,15):max(age-10,15)])
        #because data is out of the sample
        else: 
            younger_gen[age] = 0
...
def pivot_F_M_proportions(df_F_F_pivot, df_F_M_pivot):
    ...
    df_F_M_proportion =  df_F_M_proportion.loc[:,15.0:50.0].div( df_F_M_proportion['T_total'],axis=0)
...
def F_M_gen_proportions(pivot_F_M_proportions):
    ...
    for age in df_pivot_F_M_proportions.index:
            same_gen[age]= sum(df_pivot_F_M_proportions.loc[age,max(age-5,15):min(age+5,50)])

    for age in df_pivot_F_M_proportions.index:
        if age <=40:
            older_gen[age]= sum(df_pivot_F_M_proportions.loc[age,age+10:min(age+20,50)])
        #because data is out of the sample
        else: 
            older_gen[age] = 0
    ...
    for age in df_pivot_F_M_proportions.index:
        if age >=25:
            younger_gen[age]= sum(df_pivot_F_M_proportions.loc[age,max(age-20,15):max(age-10,15)])
        #because data is out of the sample
        else: 
            younger_gen[age] = 0
...
def pivot_M_F(edges_w_features):
    ...
    edges_w_features_M_F=edges_w_features.loc[(edges_w_features['gender_x']==1.0) & (edges_w_features['gender_y']==0.0)]

    df_M_F= edges_w_features_M_F.groupby(["gender_x", "gender_y", "AGE_x", "AGE_y"]).agg(
        {"smaller_id": "count"}
    )
    #creating pivot table
...
def pivot_M_M(edges_w_features):
    ...
    df_M_M= edges_w_features_M_M.groupby(["gender_x", "gender_y", "AGE_x", "AGE_y"]).agg(
        {"smaller_id": "count"}
    )
    #creating pivot table
...
def pivot_M_F_proportions(df_M_F_pivot, df_M_M_pivot):
    ...
    df_M_F_proportion =  df_M_F_proportion.loc[:,15.0:50.0].div( df_M_F_proportion['T_total'],axis=0)
...
def M_F_gen_proportions(pivot_M_F_proportion):
    ...
    for age in df_pivot_M_F_proportions.index:
            same_gen[age]= sum(df_pivot_M_F_proportions.loc[age,max(age-5,15):min(age+5,50)])

    for age in df_pivot_M_F_proportions.index:
        if age <=40:
            older_gen[age]= sum(df_pivot_M_F_proportions.loc[age,age+10:min(age+20,50)])
        #because data is out of the sample
        else: 
            older_gen[age] = 0
    ...
    for age in df_pivot_M_F_proportions.index:
        if age >=25:
            younger_gen[age]= sum(df_pivot_M_F_proportions.loc[age,max(age-20,15):max(age-10,15)])
        #because data is out of the sample
        else: 
            younger_gen[age] = 0
...
def pivot_M_M_proportions(df_M_M_pivot, df_M_F_pivot):
    ...
    df_M_M_proportion =  df_M_M_proportion.loc[:,15.0:50.0].div( df_M_M_proportion['T_total'],axis=0)
...
def M_M_gen_proportions(pivot_M_M_proportions):
    ...
    for age in df_pivot_M_M_proportions.index:
            same_gen[age]= sum(df_pivot_M_M_proportions.loc[age,max(age-5,15):min(age+5,50)])

    for age in df_pivot_M_M_proportions.index:
        if age <=40:
            older_gen[age]= sum(df_pivot_M_M_proportions.loc[age,age+10:min(age+20,50)])
        #because data is out of the sample
        else: 
            older_gen[age] = 0
    ...
    for age in df_pivot_M_M_proportions.index:
        if age >=25:
            younger_gen[age]= sum(df_pivot_M_M_proportions.loc[age,max(age-20,15):max(age-10,15)])
        #because data is out of the sample
        else: 
            younger_gen[age] = 0
...
def creating_figure_six(edges_w_features):
    ...
    ax1.legend(['F(x-5:x+5)',"F(x+10:x+20)","F(x-20:x-10)","M(x-5:x+5)","M(x+10:x+20)","M(x-20:x-10)"])
    ...
    ax2.legend(['F(x-5:x+5)',"F(x+10:x+20)","F(x-20:x-10)","M(x-5:x+5)","M(x+10:x+20)","M(x-20:x-10)"])